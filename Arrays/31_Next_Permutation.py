...
def next_permutation(nums):
    """ We observe that for any given sequence that is in descending order, no next larger permutation is possible.
        ...
        We need to find the first pair of two successive numbers nums[i] and nums[i−1], from the right, which satisfies
        ...
        larger than the current one. Therefore, we need to replace nums[i-1] with the number which is just larger than
        itself among the numbers lying to its right section, say nums[j].

        We swap the numbers nums[i−1] and nums[j]. We now have the correct number at index i-1. But still the current
        permutation isn't the permutation that we are looking for. We need the smallest permutation that can be formed
        by using the numbers only to the right of nums[i−1]. Therefore, we need to place those numbers in ascending
        ...
        Therefore, we simply need to reverse the numbers to the right of nums[i−1] to get the next smallest
        ...
        Specifically, we start from the right, and look at the longest decreasing suffix, which is (5,4,3,0). We cannot
        get the next permutation just by modifying this suffix, since it is already the maximum it can be. Instead, we
        look at the entry e that appears just before the longest decreasing suffix, which is 1 in this case. If there's
        no such element, i.e., the longest decreasing suffix is the entire permutation, reverse the entire permutation.

        Observe that e must be less than some entries in the suffix (since the entry immediately after e is greater than
        e). Intuitively, we should swap e with the smallest entry s in the suffix which is larger than e to minimize the
        change to the prefix.

        For this example, e is 1 and s is 3. Swapping s and e results in (6,2,3,5,4,1,0)
        ...
        in the suffix from smallest to largest. For this example, this yields the suffix (0,1,4,5).

        As an optimization, it is not necessary to call a full-blown sorting algorithm on the suffix. Since the suffix
        was initially decreasing, and after replacing s by e it remains decreasing, reversing the suffix has the effect
        of sorting it from smallest to largest.

        Summary: The general algorithm for computing the next permutation is as follows:
    ...
    n = len(nums)
    i = n - 1
    while i > 0 and nums[i] <= nums[i-1]:  # Look for the longest decreasing suffix
        i -= 1
    if i == 0:
        # If the entire sequence is decreasing, then the current permutation is the last in order. Reverse it.
        nums.reverse()
        return nums
    j = i - 1  # This is the index just before the starting index of the longest decreasing suffix
    k = n - 1
    while nums[k] <= nums[j]:
        # Look for the smallest element greater than the value at j. We want to increase the permutation by as
        # little as possible
        k -= 1
    nums[j], nums[k] = nums[k], nums[j]
    left, right = i, n - 1
    while left < right:  # Reverse the suffix
