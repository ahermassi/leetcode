...
def str_str_v2(haystack, needle):
    """ Rabin-Karp Algorithm.
         ...
         One example is to define the hash function as the sum of mapped values of all the characters in the string. The
         ...
         To avoid spurious hits, we can assign position weight to each index of the string. For example, the last
         character of the string is assigned a weight of 1, the second to last character of the string is assigned a
         weight of 10, the third to last character of the string is assigned a weight of 10^2, and so on.

         In this way, 'aca' will be mapped to 1*10^2 + 3*10^1 + 1*10^0 = 100 + 30 + 1 = 131, while 'abb' will be mapped
         to 1*10^2 + 2*10^1 + 2*10^0 = 100 + 20 + 2 = 122. Thus, both strings, which were mapped to 5 using the
         traditional hash, are now mapped to different values, thereby reducing the number of spurious hits.

         However, the approach is still not foolproof. 'aal' will also be mapped to 1*10^2 + 1*10^1 + 12*10^0 =
         100 + 10 + 12 = 122. Thus, we still can have spurious hits. The reason being the chosen position weight 10 is
         not enough to avoid spurious hits.

         Mathematically, it turns out that to have a unique hash value for every M-substring, positional weight should
         be greater than or equal to the number of characters in the set, which is 26 in this case. Any number
         ...
         Let M be the length of the needle. The algorithm computes the hash code of each substring of length M. The key
         to efficiency is using an incremental hash function, such as a function with the property that the hash code of
         a string is an additive function of each individual character. Such a hash function is sometimes referred to as
         a rolling hash.
         ...
         value of the first M-substring of haystack and needle. For the remaining M-substrings of haystack, we can:
            ...
            - Multiply the hash value of the previous M-substring by radix/base 26
            - Add the value of the last/current character of the current M-substring
            - Subtract the value of the first/leftmost character of the current M-substring, since we are sliding the
               window by one character, and that character is now outside the window.

        This can be simplified as:

                        H[i+1]= H[i] * b + S[i+1] − S[i-m] * b^m

        The three terms can be interpreted as:

            - H[i] * b is left shifting one radix
            - S[i+1] is the rightmost character with weight 1, which is now in the window, hence added.
            - S[i-m] * b^m is the leftmost character with modified weight which is now outside the window, hence
               subtracted.

        Thus, from the previous hash value, we can compute the current hash value in O(1) time. If the hash value of the
        current haystack window matches the hash value of the needle, then return the window's start index.

        Example: consider the string 'abcd' -> [ord('a'), ord('b'), ord('c'), ord('d')] as a number in a numeral system
        with base 26. Hence, 'abcd' could be hashed as:
            h = ord('a') * 26^3 + ord('b') * 26^2 + ord('c') * 26^1 + ord('d') * 26^0
        Now let's consider the sliding window 'abcd' -> 'bcde'. For the previous array that means:
        [ord('a'), ord('b'), ord('c'), ord('d')] -> [ord('b'), ord('c'), ord('d'), ord('e')]
            h = h * 26 + ord('e') * 26^0 - ord('a') * 26^4
        Hash regeneration is perfect and fits in constant time.
    ...
    for i in range(m, n):
        ...
        # Derive the rolling hash based on the previous hash value: multiply previous hash by base, add the hash of the
        # rightmost (new) element of the window, and subtract the leftmost element of the window.
        previous_window_start = i - m
        rolling_hash = rolling_hash * base + ord(haystack[i]) - ord(haystack[previous_window_start]) * pow(base, m)
        if needle_hash == rolling_hash:
            return previous_window_start + 1
    return -1
