...
def combination_sum(candidates, target):
    """  This is an extension or variation of the earlier problem 39- Combination Sum. Despite all the differences,
        ...
        some tricks with the index of the iteration in order to avoid generating duplicate combinations.

        Once we sort the input array, the occurrences of each number would be adjacent to each other. When we want to
        move the index forward, in order to choose the next number to be added to the combination, we need to skip
    ...
    def dfs(index, remaining, combination):
        if remaining == 0:
