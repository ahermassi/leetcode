...
def generate_parenthesis_v1(n):
    """ Instead of adding '(' or ')' every time, let's only add them when we know it will remain a valid sequence.
        ...
        We can start an opening bracket if we still have one (of n) left to place. We can start a closing bracket if it
    ...
    def dfs(open, close, path):
        ...
        if close < open:  # Can we insert a right parenthesis? Only if the number of placed left parenthesis is greater
