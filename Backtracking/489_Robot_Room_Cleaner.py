...
def clean_room(robot):
    """ This solution is based on the same idea as a maze-solving algorithm called right-hand rule. Go forward,
        ...
        explored direction.
        When to stop ? Stop when you explored all possible paths, i.e. all 4 directions (up, right, down, and left)
        for each visited cell.

        Even though we do not have a matrix given and do not know where the robot should start, we can use relative
        coordinates to represent the positions. We can assume the current cell that the robot is at is (0, 0), then
        the cell to its left will be (0, -1), and the cell above it will be (-1, 0), etc. Then, we should also use a
        hash set to keep track of the cells that the robot has already visited.

        For any of the location our robot is currently at, it has 4 choices to expand: up, right, down, left.
        Note that in order to move left, we need to make sure that the robot is currently FACING left before we call
        move(). Therefore:

            The core of this problem is that we need to know the direction the robot is facing before we move to
        ...
        When the robot keeps moving left, but then hits an obstacle or runs into a cell that has been visited before,
        what can we do?
        We try to ask the robot to make a turn so that it will face towards another direction, then it can keep moving.
        Since the robot can't move when it's facing left, we can first ask the robot to make a right turn once, so it
        now is facing up, and we should now check if it can go up. If it can, then go up. We can keep asking the robot
        to make right turns until all the possible directions (up, right, down) are explored. Finally, when all the
        directions are tried, the robot should be facing the direction that it originally was, which is left in our
        case.

        What happens if the robot has exhausted all the directions that it can visit?
        If this is the case, we should go back to the cell that we originally came from. As mentioned above, after
        trying out all the possible directions, the robot will be facing the direction that it originally was.
        This is where we do backtracking to restore to the original state. Therefore, what we can do is to ask the
        robot to make 2 right turns, and then move once. However, we should also make sure that this robot is facing
        the exact same direction it started with, so we should make another 2 right turns.
        ...
        direction is 3 (facing left), when we add 1 (turn right once), the direction index becomes 4. However, the
        correct index should be 0 (up), since now the robot is facing up. That's why we introduce the modulo operation
        here to make sure that the direction always falls into the range [0..3].

        We want to try all 4 directions, facing_direction is the index of the next direction vector since we are
        turning the robot right once every iteration (which is why the order is {-1,0},{0,1},{1,0},{0,-1}. Turning
        ...
        which is Up, but what we need to do is to continue in clockwise fashion from the current direction.
    ...
    def dfs(i, j, facing_direction):
        ...
        for k in range(4):
            ...
            if (next_i, next_j) not in visited and robot.move():  # Next cell has not been visited and is accessible
                ...
                # do a backtracking here to ask the robot to return back to (i, j)
                robot.turnRight()
                robot.turnRight()
                robot.move()
                robot.turnRight()
