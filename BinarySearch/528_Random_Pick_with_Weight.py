...
class Solution(object):
    """ To understand the problem better, let us imagine that there is a line in the space, we then project each index
         ...
         The intuition behind this solution is the following:
                        ...
                        If we have an array [a, b, c], then we can visualize this same array as:
                        ([0] * a) + ([1] * b) + ([2] * c),
        ...
        where 0, 1, 2 are the indices of the array. That's how the weights a, b, c affect the random choice of an index:
         ...
         Example: w = [2, 5, 3, 4] -> prefix_sum = [2, 7, 10, 14]
         index:      0   1    2     3
                       [   |      |     |        ]
        prefix     1   2     7   10      14
        [1, 2) region belongs to index 0, [3,7) region belongs to 1 ... etc. As we can see, the size of the region that
        each index gets is proportional to its weight.
