...
def snakes_and_ladders(board):
    """ We can model the grid as a graph. Each square is a node. There are edges between squares within 6 of each other,
         ...
         The problem is asking for the minimum number of moves, which suggests this is a shortest-path problem:
        ...
        vertices to the queue. The algorithm terminates when it has visited all vertices.
            ...
            - While the queue is not empty:
                    ...
                    * Pop a cell from the queue, cur. For each square next_square in range [(cur+1), min(curr+6,n^2)],
                       if next_square has a snake or a ladder, set next_square to the destination of that snake or ladder.

                    * If next_square has not been visited yet, increment the number of moves and push it along
                       next_square to the queue.
