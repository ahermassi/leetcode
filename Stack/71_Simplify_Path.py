...
def simplify_path_v1(path):
    """  Let's work our way through the problem to understand why a stack fits in here.

         Suppose that to a path like '/a/b/c', we add another component like '/a/b/c/..' Now, this is interesting
         because the '..' is no longer a subdirectory name. It has a special meaning and an indication to the operating
         ...
         current directory, so nothing changes in the overall path as such.
            ...
            - Split the input string using '/' as a delimiter. This step is really important because no matter what, the
               given input is a valid path, and we simply have to shorten it. So, that means that whatever we have
               between two '/' characters is either a directory name or a special character, and we have to process them
               accordingly.

            - Once we are done splitting the input path, we process one component at a time.

            - If the current component is a '.' or an empty string, we do nothing and simply continue. If you think
            ...
            - If we encounter a double-dot '..', we have to do some processing. This simply means go one level up in the
               current directory path. So, we pop an entry from the stack if it's not empty.

            - If the current component is not one of the special characters, then we simply add it to the stack because
               it's a legitimate directory name.

        Once we are done processing all the components, we simply have to connect all the directory names in the stack
        together using '/' as delimiter, and we will have the shortest path that leads us to the same directory as the
        one provided in the input.
