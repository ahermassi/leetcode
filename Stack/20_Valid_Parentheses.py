...
def is_valid_v1(s):
    """ Before looking at how we can check if a given expression consisting of these parentheses is valid or not, let
         us look at a simpler version of the problem that consists of only one type of parenthesis.
         ...
         If we encounter a closing bracket, this has two meanings:
            ...
            - We had some unmatched opening brackets available to match this closing bracket. This is the case when
        ...
        If at the end we still have unmatched left parenthesis available, this implies an invalid expression.

        If we try and follow the same approach for the original problem, then it simply won't work. The reason a
        simple counter based approach works above is because all the parenthesis are of the same type. So when we
        encounter a closing bracket, we can assume a corresponding opening matching bracket to be available i.e.
        left > 0. But, in this problem, if we encounter say ], we don't really know if there is a corresponding opening
        [ available or not.
        ...
        keep counters, then as soon as we encounter the closing square bracket ], we would know there is an unmatched
        opening square bracket available as well. But, the closest unmatched opening bracket available is a curly
        bracket { and not a square bracket [, and hence the counting approach breaks.

        An interesting property about a valid parenthesis expression is that a subexpression of a valid expression
        should also be a valid expression (not every subexpression).

        What if whenever we encounter a matching pair of parenthesis in the expression, we simply remove it from the
        expression? If it is a valid expression, we would be left with an empty string at the end.
        ...
        If we encounter an opening bracket, we simply push it to the stack. This means we will process it later, let us
        simply move on to the subexpression ahead.

        If we encounter a closing bracket, then we check the element at top of the stack. If it is an opening bracket of
        the same type, then we pop it off the stack and continue processing. Otherwise, this implies an invalid
        expression.

        At the end, if we are left with a non-empty stack, then this implies an invalid expression.

    Time complexity: O(N)
    Space complexity: O(N), we push all opening brackets to the stack and in the worst case we will end up pushing
    all the brackets, e.g. ((((((((((
    """
    brackets = {'(': ')', '{': '}', '[': ']'}
    stack = []
    for c in s:
