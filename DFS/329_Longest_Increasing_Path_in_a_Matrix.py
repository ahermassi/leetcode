...
def longest_increasing_path_v2(matrix):
    """ The result of each cell only related to the result of its neighbors. Can we use dynamic programming?
        ...
        This formula is the same as used in the previous approaches. With such transition function, we may think that
        ...
        The idea is that in a DAG, we will have some vertex that doesn't depend on others which we call 'leaves'.
        We put these leaves in a list (their internal ordering does matter), and then we remove them from the DAG.
        After the removal, there will be new leaves. We do the same repeatedly as if we are peeling an onion layer by
        ...
        Thus, we can count the number of layers during 'peeling' and return the counts in the end.
        The logic is similar to 210- Course Schedule II.
    Time complexity: O(N * M), the the topological sort is O(V + E) = O(N * M). V is the total number of vertices and
