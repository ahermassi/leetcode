...
def shortest_distance_v1(maze, start, destination):
    """" BFS. DFS version is below but it TLEs.
         Same logic as in 490- The Maze, but we also use a 'distance' array initialized with float('inf').
         distance[i][j] represents the minimum number of steps required to reach the position (i,j) from the start
         position, such that distance[start[0]][start[1]] = 0
         ...
         we check if the current path takes less steps to reach (k,l) than any other previous path taken to reach
         ...
         less number of steps. Thus, we need to update the value of distance[k][l] to distance[i][j] + d. Further, now
    ...
    while queue:
        ...
        for x, y in (-1, 0), (1, 0), (0, -1), (0, 1):  # The 4 possible directions: up, down, left, right respectively
            ...
            while 0 <= new_i + x < n and 0 <= new_j + y < m and maze[new_i + x][new_j + y] == 0:  # Moving
                ...
                new_i += x
                new_j += y
            if distance[i][j] + d < distance[new_i][new_j]:  # If we've just reached this position in fewer steps
