...
def merge_two_lists_v1(l1, l2):
    """ We create a dummy head and a dummy tail indicating the last node in the growing List.

        At each step, if the value at l1 is less than or equal to the value at l2, then we connect l1 to the tail node
        and advance l1. Otherwise, we do the same, but for l2. Then, regardless of which list we connected, we advance
        the tail to keep it one step behind one of the lists' heads.
        ...
        This code can be confusing because we never see dummy_head.next explicitly set to anything, but the reason is
        that both dummy_head and dummy_tail are simply references to the same object in memory.
        ...
        When we set "dummy_tail = dummy_head", we might think that all we're doing is assigning a copy of dummy_head
        to dummy_tail, and then we have two separate variables, but that is false. When we say "dummy_tail = dummy_head"
        what we're actually saying is that both of these variables are now pointing to the exact same object in memory;
        It doesn't get cloned.
        ...
        This only happens at the first pass of the while loop, because at that point, we change the references when we
        do "dummy_tail = dummy_tail.next" at the end of the while loop iteration. The variable dummy_tail changes
        references at every pass of the while loop, and that's how we actually traverse the lists - dummy_tail is always
        changed to reference the next node in the sorted list, and that reference is then what changes the .next of the
        current node, whereas dummy_head never gets set to anything else, so it's always pointing to the beginning of
        the new merged list.
