...
def has_cycle_v2(head):
    """ We traverse the linked list and record each node's reference in a hash set. If the current node is null, it
         means we have reached the end of the list, and it must be not cyclic. However, if the current node’s reference
         is in the hash set, then a cycle must exist.
