...
def min_window_v1(s, t):
    """ Similar to 438- Find All Anagrams in a String.
        ...
        The idea is a general solution for substring (longest/shortest/non-duplicate, etc):
            ...
            - As long as the counter indicates it's a valid range, shrink/expand the window until the range's validity
               ...
               value (using min()/max() accordingly).
    ...
    while right < n:
        cur_char = s[right]
        # Always decrement the count of the current character. If the character is not in t, the count drops below zero.
        # Interestingly, the count for non-target characters never gets positive because the upper loop decrements and
        # the lower loop increments the same elements.
        counter[cur_char] -= 1
