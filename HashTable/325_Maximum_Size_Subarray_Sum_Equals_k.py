...
def max_sub_array_len(nums, k):
    """ Similarly to 560- Subarray Sum Equals K, we use a hash map 'prefix_sum' to store the sum of all elements before
        ...
        Note that we need to initialize the hash map to {0: -1}. This way, whenever cur_sum = 0, the value at
        prefix_sum[0] remains -1 instead of being changed to the current index, i. We want to include consecutive
        values in the sub array that total to 0 in order to get the longest sub array count that sums up to k.
        Take the example nums = [1, -1, 5, -2 , 3] and k = 3. When we use prefix_sum = {0:-1} in the beginning, then
        at i = 1, cur_sum = 0. When we reach i = 3, cur_sum = 3. cur_sum - k = 0 and i - prefix_sum[cur_sum - k] = 4
        (which is the longest sub array length that totals k). If instead prefix_sum = {} in the beginning,
        prefix_sum[0] gets updated to i = 1 and, at i = 3, i - prefix_sum[cur_sum - k] = 2.
    Time complexity: O(N)
