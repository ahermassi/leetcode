...
def isNStraightHand_v2( hand, group_size):
    """ Similar approach but using a min heap instead of sorting the input.
        ...
        numbers. If any of the consecutive numbers is not in the priority queue, it implies the hand is invalid, and thus
        we return false.
    ...
    while counter:
        ...
        while start not in counter:  # Number is no loner available
            start = heappop(heap)
        for _ in range(group_size):
