...
def find_itinerary_v1(tickets):
    """ Hierholzer’s Algorithm.
        ...
        Since the problem asks for lexically smallest solution, we can put the neighbors in a min-heap. This way we
        always visit the smallest possible neighbor first in our trip. At each airport, given a list of possible
        destinations, while backtracking, at each step we would pick the destination greedily in lexical order, i.e.
        the one with the smallest lexical order would have its trial first. With this greedy strategy, we would ensure
        that the final solution that we find would have the smallest lexical order.
    ...
    Time complexity: O(E logE), where E is the number of tickets/edges
    Space complexity: O(|E| + |V|), where ∣E∣ is the number of flights and ∣V∣ is the number of airports
    """
    ...
    def dfs(airport):
        ...
        # flights (empty 'destinations' heap), but in reality this airport should be the last to visit. This is why we
